...
boolean hIsHIGH = false;
boolean jIsHIGH = false;
boolean kIsHIGH = false;
boolean lIsHIGH = false;
...
//  if (digitalRead(2) == LOW) OpenBCI_version = OPENBCI_V1; //check pins to see if there is a jumper.  if so, it is the older board
  ...
  delay(1000);
  Serial.println();
  Serial.println("+++ NEURO BATTLE BOTS +++");
  Serial.println();
...
//  pinMode(7,OUTPUT);  digitalWrite(7,LOW); //
  ...
  ADSMdanager.configureLeadOffDetection(LOFF_MAG_6NA, LOFF_FREQ_31p2HZ);
